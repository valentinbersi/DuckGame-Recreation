...
const ReplyMessage ReplyMessage::startGameInstance =
        ReplyMessage(NO_MATCHID, START_GAME, NO_CONNECTED_PLAYERS, DuckData::Id::None,
                     DuckData::Id::None, NO_ERROR);
...
ReplyMessage::ReplyMessage(const std::string& error):
        ServerMessage(MessageType::Reply),
        matchID(NO_MATCHID),
        startGame(NO_START_GAME),
        connectedPlayers(NO_CONNECTED_PLAYERS),
...
ReplyMessage::ReplyMessage(u16 id, u8 startGame, u8 connectedPlayers, DuckData::Id color1,
...
bool ReplyMessage::operator==(const ReplyMessage& other) const {
    return type == other.type && matchID == other.matchID && startGame == other.startGame &&
           connectedPlayers == other.connectedPlayers && color1 == other.color1 &&
           color2 == other.color2 && error == other.error;
}
