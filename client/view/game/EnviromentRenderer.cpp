...
void EnviromentRenderer::drawEnviroment(SDL2pp::Rect& position, const char* path) const {
...
void EnviromentRenderer::drawBullets(
        const std::list<std::pair<Vector2, Vector2>>& bulletPositions) const {
    for (const auto& position: bulletPositions) {
        SDL_SetRenderDrawColor(renderer.Get(), 255, 255, 255, 255);  // Color blanco
        SDL_RenderDrawLine(renderer.Get(), position.first.x(), position.first.y(),
                           position.second.x(), position.second.y());
    }
}
