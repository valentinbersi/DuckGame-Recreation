...
GameMapMonitor::NoSpaceLeft::NoSpaceLeft(u16 MatchID): std::logic_error("Match " + std::to_string(MatchID) + " is full") {}

GameMapMonitor::AlreadyStarted::AlreadyStarted(u16 MatchID): std::logic_error("Match " + std::to_string(MatchID) + " was already started") {}

GameMapMonitor::MatchEnded::MatchEnded(u16 MatchID): std::logic_error("Match " + std::to_string(MatchID) + " ended") {}
...
BlockingQueue<std::unique_ptr<Command>>* GameMapMonitor::joinGame(
    ...
    std::vector<DuckData::Id> colors(playerCount);
...
void GameMapMonitor::createGameAndJoinSafe(std::shared_ptr<BlockingQueue<std::shared_ptr<ServerMessage>>> senderQueue,
            u16 clientId, u8 playerCount) {
    std::lock_guard lock(mutex);
    u16 random_number;
    do {
