...
TEST(ProtocolTest, ServerToClientSendOneStatus){
    ...
    status->gameObjects.emplace_back(creatDuckData());
...
TEST(ProtocolTest, ServerToClientCorrectVector){
    ListenerSocket peer("8080");
    ...
    std::thread client ([status](){
        ...
        ASSERT_TRUE(duckdata->id == (status->gameObjects.front())->id);
